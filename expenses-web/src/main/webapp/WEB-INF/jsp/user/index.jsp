...
<form class="form-inline" role="form" style="float:left;">
  <div class="form-group has-feedback">
    <div class="input-group">
      <div class="input-group-addon">查询账号</div>
	...
	<script src="${APP_PATH}/script/clickActive.js"></script>
        ...
        <script type="text/javascript">
					...
					success:function(result){
						...
						if(result.success){
							...
							$.each(users, function(i,user){
								...
								tableContent += '	<td>'+user.jobnum+'</td>';
								tableContent += '	<td>'+user.loginacct+'</td>';
								tableContent += '	<td>'+user.username+'</td>';
								tableContent += '	<td>'+user.sex+'</td>';
								tableContent += '	<td>'+user.agency+'</td>';
								tableContent += '	<td>'+getMyDate(user.startdate)+'</td>';
								tableContent += '	<td>'+getMyDate(user.updatedate)+'</td>';
								tableContent += '	<td>';
							...
							for( var i= 1;i<=userPage.totalpn;i++){
								if(i == pn){
            ...
            function deleteUser(id,loginacct){
            	layer.confirm("是否要删除【"+loginacct+"】",  {icon: 3, title:'提示'}, function(cindex){
